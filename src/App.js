...
const App = () => {
  ...
  const generateCircles = useCallback(() => {
    const newCircles = []; // Khởi tạo mảng mới ở đây
    for (let i = 0; i < points; i++) {
      ...
      newCircles.push({ id: points - i, x, y }); // Thêm vòng tròn vào mảng
    }
  ...
  const handleCircleClick = useCallback(
    (id) => {
      if (!clickCircleRef.current[id]) {
        clickCircleRef.current[id] = id; // Simplified assignment
        ...
        if (id === nextNumber) {
          if (nextNumber === points) {
            const TIME_END = TIME_DELAY_COUNT * (TIME_COUNT_DOWN / STEP_TIME_COUNT_DOWN + 6); //time from click to hidden circle
